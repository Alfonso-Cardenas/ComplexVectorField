...
def render_experiment(trajectories, color_matrix, line_colors, line_connections):
    ...
    attraction_zone_sample = np.load('attraction_zone_sample.npy')
    attraction_zone_colors = np.load('attraction_zone_colors.npy')
    atraction_zone_points = plot.Scatter3D(
        pos = helper_functions.sterographic_projection(attraction_zone_sample),
        face_color=attraction_zone_colors,
        symbol="o",
        size=4,
        edge_color=attraction_zone_colors,
    )
    object_list.append(atraction_zone_points)
    binding_list.append(bindings.Toggle([atraction_zone_points], 'o'))
